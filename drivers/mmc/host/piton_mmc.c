...
static void piton_mmc_request(struct mmc_host *mmc, struct mmc_request *req)
{
    ...
    if (data == NULL) {
		dev_info(dev, "data null; piton_mmc_request: cmd->opcode=%d, cmd->arg=%d, cmd->flags=%d\n", cmd->opcode, cmd->arg, cmd->flags);
		cmd->error = 0;
		if (cmd->opcode == MMC_APP_CMD) {
			cmd->resp[0] = R1_APP_CMD;
		}
		if (cmd->opcode == MMC_SEND_OP_COND){
			dev_info(dev, "received MMC_SEND_OP_COND, resp = %d\n", mmc->ocr_avail);
			cmd->resp[0] = mmc->ocr_avail;
		}
		if (cmd->opcode == 41){
			dev_info(dev, "received 41, resp = %d\n", mmc->ocr_avail);
			cmd->resp[0] = mmc->ocr_avail;
		}
    ...
    dev_info(dev, "piton_mmc_request: cmd->opcode=%d, cmd->arg=%d, data->blocks=%d, data->blksz=%d, data->flags=%d, rw_op: %lx\n",
    ...
    for_each_sg(data->sg, sg, data->sg_len, i) {
        ...
        dev_info(dev, "piton_mmc_request: sg[%d]: %p, %d\n", i, sg, sg->length);
        if (sg_is_last(sg)) {
    ...
    if (host->bytes_remain != host->buf_size) {
        dev_info(dev, "piton_mmc_request: bytes_remain: %lx, buf_size: %lx\n", host->bytes_remain, host->buf_size);
    }
